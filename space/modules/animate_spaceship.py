...
async def animate_spaceship(canvas, start_row, start_column, file_contents):
    ...
    for step_text in cycle(file_contents):
        ...
        for _ in range(2):
            ...
            start_row = max(1, min(start_row + rows_direction, max_rows-frame_to_size[step_text][ROW_INDEX]-1))
            start_column = max(1, min(start_column + columns_direction, max_cols-frame_to_size[step_text][COL_INDEX]-1))
